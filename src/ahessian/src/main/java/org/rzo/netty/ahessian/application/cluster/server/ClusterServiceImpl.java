...
public class ClusterServiceImpl implements SeedClusterService
{
	...
	@Override
	synchronized public void join(String clusterName, String clientName, Object data, ClusterEventListener listener)
	{
		...
		if (channel == null)
			throw new RuntimeException("join: could not get the channel for "+_clusterName+"/"+clientName);
	...
	synchronized public void seedJoin(Member member, ClusterEventListener listener)
	{
		if (member.isSeed() && _joinedSeeds.containsKey(member.getName()))
			throw new RuntimeException("already joined "+_clusterName+"/"+member.getName());
	...
	synchronized protected Member createMember(String clusterName, String clientName, Object data, boolean isSeed, String host)
	{
		...
		if (!_clusterName.equals(clusterName))
			throw new RuntimeException("cannot join wrong cluster "+_clusterName +"!=" +clusterName+"/"+clientName);
	...
	synchronized public void leave(String clientName)
	{
		...
		if ( member == null)
			throw new RuntimeException("cannot leave not joined cluster "+_clusterName+"/"+clientName);
