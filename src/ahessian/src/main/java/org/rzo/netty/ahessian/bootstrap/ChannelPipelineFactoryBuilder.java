...
public class ChannelPipelineFactoryBuilder<T> implements ChannelPipelineFactoryFactory
{
	enum Encryption
	{
		MD5,
	...
	public ChannelPipelineFactoryBuilder connectedListener(ConnectListener listener)
	{
		_connectedListener = listener;
		return this;
	}

	public ChannelPipelineFactoryBuilder disconnectedListener(ConnectListener listener)
	{
			...
			AbstractBootstrap bootstrap)
	{
		...
		{
        _serverFactory =  new HessianRPCServiceHandler(Executors.newCachedThreadPool(), _serviceOptions, TIMER);
        ...
        if (isExecutorService())
        	_serverFactory.addService("default", new ExecutorInvokeService(_serverService, _serverServiceInterface, _serverFactory, Executors.newFixedThreadPool(_executorThreads)));
        else
        	_serverFactory.addService("default", new ImmediateInvokeService(_serverService, _serverServiceInterface, _serverFactory));
		}
		else if (isRPC() && _clientFactory == null)
		{
			Executor executor = _executorThreads > 0 ? Executors.newFixedThreadPool(_executorThreads) : Executors.newSingleThreadExecutor();
			String name = "NONAME?";
			_clientFactory = new HessianProxyFactory(executor, name, _serviceOptions);
		}

		if (hasSession())
		{
			result =  basePipelineFactory(mixinPipelineFactory(group, bootstrap), group, bootstrap);
		}
		else
			result =  mergePipelineFactory(basePipelineFactory(group, bootstrap), mixinPipelineFactory(group, bootstrap));
	...
	private ChannelPipelineFactory basePipelineFactory(ChannelPipelineFactory mixinPipelineFactory, EventLoopGroup group, final AbstractBootstrap bootstrap)
	{
		final ClientSessionFilter clientSessionFilter = mixinPipelineFactory != null ?
			...
			:
				null;
				final ServerSessionFilter serverSessionFilter = mixinPipelineFactory != null ?
						new ServerSessionFilter(mixinPipelineFactory, TIMER, _sessionTimeout)
					:
						null;
		...
		ChannelPipelineFactory result = new ChannelPipelineFactory(group)
		{
			...
			public HandlerList getPipeline() throws Exception
			{
					...
					else if (hasAuthentication())
					{
						...
						if (_passwordEncryption.equals(Encryption.NONE))
							...
							token = new Base64AuthToken("", _passwords[0]);
								...
								((EncryptedAuthToken)token).setPassword(_passwords[0]);
								((EncryptedAuthToken)token).setLength(20);
								}
						if (token != null)
							tokens.add(token);
						}
						AuthToken tokenList = AuthTokenList.fromList(tokens, _unique);
				...
				else // client
				{
					if (hasReconnect())
					{
					    pipeline.addLast("reconnect", new ReconnectHandler(new BootstrapProvider()
					    {
	...
	{
		return _passwords != null && _passwords.length > 0 && _passwordEncryption != null;
	}
		...
		ChannelPipelineFactory result = new ChannelPipelineFactory(group)
		{
			...
			public HandlerList getPipeline() throws Exception
			{
				...
				if (isRPC())
				{
				    ...
				    //ChannelHandlerInvoker invoker = new DirectWriteChannelHandlerInvoker(loop);
					if (isRPCServer())
					{
						//pipeline.addLast("xlogger-inputStream",new OutLogger("inputStream"));
						pipeline.addLast("inputStream", new InputStreamHandler());
						//pipeline.addLast("xlogger-outputStream",new OutLogger("outputStream"));
						pipeline.addLast("outputStream", new OutputStreamHandler(), getGroup());
						//pipeline.addLast("xlogger-callDecoder",new OutLogger("callDecoder"), getGroup());
				        pipeline.addLast("callDecoder", new PullInputStreamConsumer(new HessianRPCCallDecoder(_serializerFactory)), getGroup());
				        //pipeline.addLast("xlogger-replyEncoder",new OutLogger("replyEncoder"),getGroup());
				        pipeline.addLast("replyEncoder", new HessianRPCReplyEncoder(_serializerFactory, _executor), getGroup());
				        //pipeline.addLast("xlogger-hessianRPCServer",new OutLogger("hessianRPCServer"), getGroup());
				        pipeline.addLast("hessianRPCServer", _serverFactory, getGroup());	
				        //pipeline.addLast("xlogger-stop",new OutLogger("stop"), getGroup());
				        pipeline.addLast("stop", new StopHandler(), getGroup());
					}
					else
					{
						_clientFactory.setConnectedListener(_connectedListener);
						_clientFactory.setDisconnectedListener(_disconnectedListener);

						//pipeline.addLast("xlogger-inputStream",new OutLogger("inputStream"));
				        pipeline.addLast("inputStream", new InputStreamHandler());
						//pipeline.addLast("xlogger-outputStream",new OutLogger("outputStream"), getGroup());
				        pipeline.addLast("outputStream", new OutputStreamHandler(), getGroup());
				        //pipeline.addLast("xlogger-replyDecoder",new OutLogger("replyDecoder"), getGroup());
				        pipeline.addLast("replyDecoder", new PullInputStreamConsumer(new HessianRPCReplyDecoder(_clientFactory, _serializerFactory)), getGroup());
				        //pipeline.addLast("xlogger-callEncoder",new OutLogger("callEncoder"), getGroup());
				        pipeline.addLast("callEncoder", new HessianRPCCallEncoder(_inverseServer, _serializerFactory, _executor), getGroup());
				        //pipeline.addLast("xlogger-hessianRPCClient",new OutLogger("hessianRPCClient"), getGroup());
				        pipeline.addLast("hessianRPCClient", _clientFactory, getGroup());
				        //pipeline.addLast("xlogger-stop",new OutLogger("stop"), getGroup());
				        pipeline.addLast("stop", new StopHandler(), getGroup());
					}
	...
	public T proxy()
	{
		if (_proxy == null)
		{
			...
			_proxy = (T) _clientFactory.create(_serverServiceInterface, this.getClass().getClassLoader(), _serviceOptions);
		}
		return _proxy;
	}
	public void close()
