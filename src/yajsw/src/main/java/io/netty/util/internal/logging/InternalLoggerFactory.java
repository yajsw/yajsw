...
public abstract class InternalLoggerFactory
{
	...
	static
	{
		...
		// yajsw: use jdkLogger2
		/*
		 * try { f = new Slf4JLoggerFactory(true);
		 * f.newInstance(name).debug("Using SLF4J as the default logging framework"
		 * ); defaultFactory = f; } catch (Throwable t1) { try { f = new
		 * Log4JLoggerFactory();
		 * f.newInstance(name).debug("Using Log4J as the default logging framework"
		 * ); } catch (Throwable t2) { f = new JdkLoggerFactory();
		 * f.newInstance(
		 * name).debug("Using java.util.logging as the default logging framework"
		 * ); } }
		 */
		// default logging in netty -> do not log debug, trace
		// f = new SimpleErrorLoggerFactory();
		f = new SimpleErrorLoggerFactory();
		defaultFactory = f;
	}
